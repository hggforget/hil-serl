...
class BSACAgent(flax.struct.PyTreeNode):
    """
    ...
    def critic_loss_fn(self, batch, params: Params, rng: PRNGKey):
        ...
        info = {
            ...
            **tensorstats(predicted_qs, 'predicted_qs'),
            **tensorstats(target_qs, 'target_qs'),
            "rewards": batch["rewards"].mean(),
        }

        return critic_loss, info

    def policy_loss_fn(self, batch, online_batch, demo_batch, params: Params, rng: PRNGKey):
        batch_size = batch["rewards"].shape[0]
        temperature = self.forward_temperature()
        ...
        rl_loss = -jnp.mean(actor_objective)
    
        pi_qs = self.forward_critic(
        online_batch["observations"],
        self.sample_actions(online_batch["observations"], seed=sample_rng),
        rng=critic_rng,
        )
        bc_qs = self.forward_critic(
        online_batch["observations"],
        self.teacher_agent.sample_actions(online_batch["observations"], seed=sample_rng),
        rng=critic_rng,
        )
        bc_weight = jax.lax.stop_gradient((bc_qs > pi_qs).astype(jnp.float32).mean()) 
        bc_dist = self.forward_policy(
        demo_batch["observations"], rng=policy_rng, grad_params=params
        )
        regularization_loss = -jnp.mean(bc_dist.log_prob(demo_batch["actions"])) * 0.3
        # actor_loss = rl_loss + regularization_loss
        actor_loss = rl_loss
        info = {
            **tensorstats(actions, prefix='action'),
            "temperature": temperature,
            'actor_loss': actor_loss,
            'rl_loss': rl_loss,
            'bc_weight': bc_weight,
            'regularization_loss': regularization_loss,
            **tensorstats(-log_probs, prefix='entropy'),
        }
    ...
    def loss_fns(self, batch, online_batch, demo_batch, **kwargs):
        return {
            "critic": partial(self.critic_loss_fn, batch),
            "actor": partial(self.policy_loss_fn, batch, online_batch, demo_batch),
    ...
    def update(
        ...
        **kwargs
        ...
        if self.config["image_keys"][0] not in batch["next_observations"]:
            batch = _unpack(batch)
            online_batch = _unpack(online_batch)
            demo_batch = _unpack(demo_batch)
        ...
        if "augmentation_function" in self.config.keys() and self.config["augmentation_function"] is not None:
            ...
            online_batch = self.config["augmentation_function"](online_batch, aug_rng)
            demo_batch = self.config["augmentation_function"](demo_batch, aug_rng)
        ...
        loss_fns = self.loss_fns(batch, online_batch, demo_batch, **kwargs)
    ...
    def create_pixels(
        ...
        augmentation_function: Optional[callable] = None,
        encoding_wrapper: nn.Module =EncodingWrapper,
        **kwargs,
    ):
        """
        ...
        if encoder_type == "resnet":
            ...
            actor_encoders = {
                image_key: resnetv1_configs["resnetv1-10"](
                    ...
                    bottleneck_dim=256,
                    name=f"actor_encoder_{image_key}",
                )
            ...
            critic_encoders = {
                image_key: resnetv1_configs["resnetv1-10"](
                    ...
                    num_spatial_blocks=8,
                    bottleneck_dim=256,
                    name=f"critic_encoder_{image_key}",
                )
                for image_key in image_keys
            }
            ...
            actor_encoders = {
                image_key: PreTrainedResNetEncoder(
                    ...
                    num_spatial_blocks=8,
                    bottleneck_dim=256,
                    pretrained_encoder=resnetv1_configs["resnetv1-10"](
                        pre_pooling=True,
        ...
        actor_encoder_def = encoding_wrapper(
            encoder=actor_encoders,
            use_proprio=use_proprio,
            enable_stacking=False,
            image_keys=image_keys,
        )
        critic_encoder_def = encoding_wrapper(
            encoder=critic_encoders,
            use_proprio=use_proprio,
            enable_stacking=False,
            image_keys=image_keys,
        )
