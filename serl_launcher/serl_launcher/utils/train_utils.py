...
def concat_batches(online_batch, offline_batch, axis=1):
    ...
    for k, v in online_batch.items():
        if type(v) is dict:
            batch[k] = concat_batches(online_batch[k], offline_batch[k],  axis=axis)
        else:
            batch[k] = jnp.concatenate((online_batch[k], offline_batch[k]), axis=axis)
...
def subsample(values, amount=1024):
  ...
  if len(values) > amount:
    values = jax.random.permutation(jax.random.PRNGKey(0), values)[:amount]
  return values