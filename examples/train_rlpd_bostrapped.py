...
from serl_launcher.agents.continuous.bostrapped_sac import BSACAgent
...
from serl_launcher.utils.launcher import (
    ...
    make_bostrapped_sac_pixel_agent,
    make_trainer_config,
    make_wandb_logger,
...
flags.DEFINE_boolean("save_success", False, "Save success episode.")
flags.DEFINE_boolean("bootstrap", False, "Wether to bootstrap")
...
def actor(agent, data_store, demos_data_store, env, sampling_rng):
    """
    ...
    for step in pbar:
        ...
        with timer.context("step_env"):
            ...
            if info['succeed'] == False or input(f'confirm? y/n').lower() == 'y':
                data_store.insert(transition)
                episodic_transitions.append(copy.deepcopy(transition))
            ...
            if done or truncated:
                if FLAGS.save_success and info['succeed']:
                    for trans in episodic_transitions:
                        demos_data_store.insert(trans)
                    demo_transitions.extend(episodic_transitions)
                episodic_transitions = []
...
def learner(rng, agent, replay_buffer, demo_buffer, logger=None):
    """
    ...
    if isinstance(agent, BSACAgent):
        train_critic_networks_to_update = frozenset({"critic"})
        train_networks_to_update = frozenset({"critic", "actor", "temperature"})
    else:
        raise NotImplementedError(f"Unknown agent type")
    ...
    for step in tqdm.tqdm(
        ...
        if step < FLAGS.pretrain_critic_step:
            online_batch = next(demo_iterator)
            demo_batch = next(demo_iterator)
            online_batch = online_batch.copy(
                add_or_replace={
                    'actions': online_batch['actions'] * 100.0
                }
            )
            demo_batch = demo_batch.copy(
                add_or_replace={
                    'actions': demo_batch['actions'] * 100.0
                }
            )
...
def main(_):
    ...
    assert FLAGS.bc_checkpoint_path is not None and os.path.exists(FLAGS.bc_checkpoint_path),  "BC checkpoint path is required."
    ...
    rng, sampling_rng = jax.random.split(rng)
    if config.setup_mode == 'single-arm-insprie':
        if FLAGS.bootstrap:
            agent: BSACAgent = make_bostrapped_sac_pixel_agent(
                seed=FLAGS.seed,
                sample_obs=env.observation_space.sample(),
                sample_action=env.action_space.sample(),
                teacher_agent=bc_agent,
                image_keys=config.image_keys,
                encoder_type=config.encoder_type,
    ...
    if FLAGS.checkpoint_path is not None and os.path.exists(FLAGS.checkpoint_path):
        ...
        new_state.params['modules_actor'] = ckpt.params['modules_actor']
        new_state.target_params['modules_actor'] = ckpt.target_params['modules_actor']
        agent = agent.replace(state=new_state)
        ckpt_number = os.path.basename(
            checkpoints.latest_checkpoint(os.path.abspath(FLAGS.bc_checkpoint_path))
        )[11:]
        print_green(f"Loaded pretrained checkpoint at step {ckpt_number}.")
    def create_replay_buffer_and_logger():
        ...
        return replay_buffer, logger

    if FLAGS.learner:
        sampling_rng = jax.device_put(sampling_rng, device=sharding.replicate())
        replay_buffer, logger = create_replay_buffer_and_logger()
        ...
        demos_data_store = QueuedDataStore(50000)
